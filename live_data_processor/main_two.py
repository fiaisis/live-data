...
def get_script() -> str:
    ...
    return """from mantid.simpleapi import *; SaveNexusProcessed(Filename="~/work/live-data/output-lives.nxs", InputWorkspace="lives")"""
...
def process_events(events: EventMessage, spectra_detector_map: dict[int, int]):
    """
    ...
    if len(times_of_flight) > 0:
        for detector_id, tof in zip(detector_ids, times_of_flight, strict=False):
            spectra = ws.getSpectrum(spectra_detector_map[int(detector_id)])
...
def initialize_run(events_consumer, runinfo_consumer, run_start: RunStart | None = None) -> RunStart:
    ...
    if run_start is None:
        ...
        logger.info("DET MAP")
        logger.info(str(run_start.DetectorSpectrumMap()))
        if run_start is None:
...
def start_live_reduction(
    ...
    TIME_BETWEEN_SCRIPT_RUNS = 10
    TIME_BETWEEN_CHECKS_OF_RUNINFO = 1
    TIME_BETWEEN_SCRIPT_loop_start_time = datetime.datetime.now()
    TIME_BETWEEN_CHECKS_OF_RUNINFO_loop_start_time = datetime.datetime.now()

    detector_spectrum_map: dict[int, int] = dict(
        zip(
            map(int, current_run_start.DetectorSpectrumMap().DetectorIdAsNumpy()),
            map(int, current_run_start.DetectorSpectrumMap().SpectrumAsNumpy()),
        )
    )
    for message in events_consumer:
        ...
        process_events(events, detector_spectrum_map)
        if (datetime.datetime.now() - loop_start_time).total_seconds() > float(SCRIPT_REFRESH_TIME):
            try:
            ...
            loop_start_time = datetime.datetime.now()
        if (datetime.datetime.now() - TIME_BETWEEN_SCRIPT_loop_start_time).total_seconds() > float(
            TIME_BETWEEN_SCRIPT_RUNS
        ):
            try:
                exec(script)
        ...
        if (
            datetime.datetime.now() - TIME_BETWEEN_CHECKS_OF_RUNINFO_loop_start_time
        ).total_seconds() > TIME_BETWEEN_CHECKS_OF_RUNINFO:
