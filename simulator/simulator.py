...
def build_ev42_from_dict(ev42_message: dict) -> bytes:
    """
    Build an EventMessage (ev42) FlatBuffer from a dictionary.

    This function converts a dictionary containing event data into a serialized
    FlatBuffer message using the ev42 schema.

    Parameters
    ----------
    ev42_message : dict
        Dictionary containing event data with the following keys:
        - message_id : int
            Unique identifier for the message
        - pulse_time : int
            Time of the pulse in nanoseconds
        - event_time_offset : list
            List of time offsets for each event
        - event_id : list
            List of detector IDs for each event
        - source_name : str, optional
            Name of the source, defaults to "simulator"

    Returns
    -------
    bytes
        Serialized FlatBuffer message with file identifier "ev42"
    """
...
def build_run_stop(stop_time: int, run_name: str) -> bytes:
    """
    Build a RunStop FlatBuffer message.

    This function creates a serialized FlatBuffer message for a run stop event
    using the pl72 schema.

    Parameters
    ----------
    stop_time : int
        The timestamp when the run stopped
    run_name : str
        The name of the run that is being stopped

    Returns
    -------
    bytes
        Serialized FlatBuffer message with file identifier "pl72"
    """
    builder = flatbuffers.Builder(initialSize=1024)
...
def build_run_start(start_time: int, run_name: str, instrument_name: str) -> bytes:
    """
    Build a RunStart FlatBuffer message.

    This function creates a serialized FlatBuffer message for a run start event
    using the 6s4t schema.
    ...
    run_name : str
        The name of the run that is being started
    instrument_name : str
        The name of the instrument on which the run is performed

    Returns
    -------
    bytes
        Serialized FlatBuffer message with file identifier "6s4t"
    """
    builder = flatbuffers.Builder(initialSize=1024)
...
def main() -> None:
    ...
    This function sets up a Kafka producer and simulates event data from a Nexus file.
    It performs the following steps:
    1. Creates a Kafka producer
    2. Opens a Nexus file and extracts event data
    3. Sends a RunStop message for any previous run
    4. Sends a RunStart message for the current run
    5. Iterates through event data, simulating real-time data flow
    6. Sends event messages to Kafka
    7. Sends a RunStop message when finished
    ...
    with h5py.File("MER72596.nxs", "r") as f:
        ...
        instrument = (
            f["/raw_data_1/instrument_name"][0].decode("utf-8").strip()
            if "/raw_data_1/instrument_name" in f
            else "MERLIN"
        )
        ...
        producer.send(
            f"{instrument}_runInfo", build_run_stop(stop_time=start_time, run_name=f"{instrument}_previous_run")
        )
        producer.send(
            f"{instrument}_runInfo",
            build_run_start(start_time=start_time, run_name=run_name, instrument_name=instrument),
        )
        producer.flush()
        ...
        for index, (tofs, ids, pulse_time) in enumerate(source.get_data()):
            ...
            producer.flush()
        producer.send(f"{instrument}_runInfo", build_run_stop(stop_time=end_time, run_name="Some previous run"))
        producer.flush()
